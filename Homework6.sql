...
CREATE TABLE users_old (
	id BIGINT PRIMARY KEY, -- поставила id из старой таблицы, а не автоматические, чтобы потом можно было найти, связанные с ними данные, но не совсем представляю как (ведь по сути перемещение удаляет те связи, раз удалили из таблицы users)
    firstname VARCHAR(50),
    lastname VARCHAR(50) COMMENT 'Фамилия',
    email VARCHAR(120) UNIQUE
);
